...
class LogisticRegression(object):
    def __init__(self, input, label, n_visible, in_dim, n_out,W=None,b=None,activation=sigmoid):
        self.activation = activation
        self.x = dense(input)
        self.y = label
        if W is None:
            self.W = numpy.zeros([n_visible*in_dim, n_out])  # initialize W 0
        else:
            self.W = W
        if b is None:
            ...
            self.b = b
        #print('train',self.x.shape,self.W.shape)
    def reset(self,input,label):
        self.x = dense(input)
        self.y = label
    ...
    def train(self, lr=0.1, input=None, L2_reg=0.00):
        if input is not None:
            self.x = dense(input)
        p_y_given_x = self.activation(dot(self.x, self.W) + self.b)
        d_y = self.y - p_y_given_x



        err = dot(self.x.T, d_y) - lr * L2_reg * self.W
    ...
    def predict(self, x):
        return self.activation(dot(dense(x), self.W) + self.b)
